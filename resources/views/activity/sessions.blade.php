...
@section('title', 'Login Sessions - XenonOS')
...
<header class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6 sm:mb-8 md:mb-10">
    <div class="space-y-1.5 sm:space-y-2">
        ...
        <p class="text-on-surface-variant text-xs sm:text-sm md:text-base max-w-lg leading-relaxed">Monitor active user sessions and manage login activity across devices.</p>
    </div>
...
<form method="GET" action="{{ route('activity.sessions') }}" class="bg-surface-container-low rounded-2xl sm:rounded-3xl shadow-sm p-3 sm:p-4 mb-6 sm:mb-8">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4">
        <select name="status" class="bg-surface-container text-on-surface-variant text-sm rounded-xl sm:rounded-2xl py-2.5 sm:py-3 px-4 border-0 focus:ring-2 focus:ring-primary transition-all min-h-[44px]">
        ...
        <div class="flex justify-between items-start mb-4 sm:mb-6">
            <div class="flex items-center gap-3">
                <div class="bg-surface-container-highest p-2.5 rounded-xl shrink-0">
                    <span class="material-symbols-outlined text-primary">{{ $session->device_type === 'mobile' ? 'smartphone' : ($session->device_type === 'tablet' ? 'tablet' : 'desktop_windows') }}</span>
                </div>
                <div class="min-w-0">
        ...
        <div class="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
            ...
            <div class="flex justify-between text-xs min-h-[44px] items-center">
                ...
                <span class="text-on-surface font-medium">
                    @php $lastActivity = is_numeric($session->last_activity) ? \Carbon\Carbon::createFromTimestamp($session->last_activity) : $session->last_activity; $diff = $lastActivity->diff(now()); @endphp
                    {{ $diff->d > 0 ? $diff->d . 'd ' . $diff->h . 'h' : ($diff->h > 0 ? $diff->h . 'h ' . $diff->i . 'm' : $diff->i . 'm') }}
                </span>
            </div>
        </div>
        <form method="POST" action="{{ route('sessions.forceLogout', $session->id) }}">
            @csrf @method('DELETE')
            <button type="submit" class="w-full p-2.5 sm:p-3 bg-error/10 text-error hover:bg-error/20 border border-error/20 rounded-xl text-xs font-bold uppercase tracking-widest transition-all min-h-[44px]">Force Logout</button>
        </form>
    </div>
    @empty
...
<div class="mt-6 sm:mt-8 flex justify-center">{{ $sessions->links() }}</div>
@endif
@endsection