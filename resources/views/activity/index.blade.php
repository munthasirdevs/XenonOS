...
<head>
    ...
    <title>Activity Logs - XenonOS</title>
        ...
        <div class="flex-1 w-full max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 lg:px-10 py-6 sm:py-8 md:py-10 lg:py-12">
            ...
            <form method="GET" action="{{ route('activity') }}" class="bg-surface-container-low rounded-2xl sm:rounded-3xl shadow-sm p-3 sm:p-4 mb-6 sm:mb-8">
                <div class="flex flex-wrap items-center gap-3 sm:gap-4">
                    <div class="w-full sm:flex-1 sm:min-w-[220px] relative">
                        ...
                        <input type="search" name="search" value="{{ request('search') }}" placeholder="Search User IDs, actions or keywords..."
            ...
            <section class="space-y-4 sm:space-y-6">
                <div class="flex gap-4 sm:gap-6 md:gap-8 border-b border-outline/10 overflow-x-auto pb-1">
                    <a href="{{ route('activity') }}" class="pb-3 sm:pb-4 border-b-2 border-primary text-primary text-xs sm:text-sm font-semibold whitespace-nowrap flex items-center {{ request()->routeIs('activity') && !request()->routeIs('activity.*') ? '' : 'text-on-surface-variant' }}">
                        User & System Activity
                    </a>
                    <a href="{{ route('activity.sessions') }}" class="pb-3 sm:pb-4 border-b-2 border-transparent text-on-surface-variant text-xs sm:text-sm font-medium hover:text-on-surface transition-all whitespace-nowrap flex items-center {{ request()->routeIs('activity.sessions') ? 'border-primary text-primary' : '' }}">
                        Login Sessions
                    </a>
                    <a href="{{ route('activity.security') }}" class="pb-3 sm:pb-4 border-b-2 border-transparent text-on-surface-variant text-xs sm:text-sm font-medium hover:text-on-surface transition-all whitespace-nowrap flex items-center {{ request()->routeIs('activity.security') ? 'border-primary text-primary' : '' }}">
                        Admin Controls
                    </a>
                </div>
                ...
                <div class="bg-surface-container-low rounded-2xl sm:rounded-3xl shadow-sm overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse min-w-[640px]">
                            ...
                            <tbody class="divide-y divide-outline/5">
                                        ...
                                        <div class="space-y-1">
                                            <div class="text-xs sm:text-sm text-on-surface flex items-center gap-2">
                                                <span class="material-symbols-outlined text-sm text-primary flex-shrink-0">
                                                    {{ $activity->action === 'create' ? 'add' : ($activity->action === 'delete' ? 'delete' : ($activity->action === 'update' ? 'edit' : 'sync')) }}
                                                </span>
                                                {{ $activity->description ?? $activity->action }}
                                            </div>
                                            <div class="text-[10px] text-on-surface-variant uppercase tracking-wider">
                                                Module: {{ $activity->computed_module }}
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 sm:px-6 md:px-8 py-4 sm:py-5 text-center">
                                        @php $severity = $activity->computed_severity; @endphp
                                        <span class="bg-{{ $severity === 'critical' ? 'tertiary' : ($severity === 'info' ? 'slate-500/10' : 'primary') }}/10 text-{{ $severity === 'critical' ? 'tertiary' : ($severity === 'info' ? 'slate-400' : 'primary') }} text-[10px] font-bold px-2.5 sm:px-3 py-1 rounded-full uppercase tracking-widest border border-{{ $severity === 'critical' ? 'tertiary' : ($severity === 'info' ? 'slate-500/20' : 'primary/20') }} whitespace-nowrap">
                                            {{ $severity }}
                                        </span>
                                    </td>
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-on-surface-variant">No activity logs found</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="px-4 sm:px-6 md:px-8 py-4 sm:py-6 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-outline/5">
                        <div class="text-xs text-on-surface-variant">
                            Showing <span class="text-on-surface font-bold">{{ $activities->firstItem() }}</span> to <span class="text-on-surface font-bold">{{ $activities->lastItem() }}</span> of <span class="text-on-surface font-bold">{{ $activities->total() }}</span> actions
                        </div>
